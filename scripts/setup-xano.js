...
const XANO_TABLES = {
  communications: {
    fields: [
      ...
      { name: 'member_id', type: 'integer', nullable: true, foreign_key: 'members.id' },
      ...
      { name: 'ai_confidence', type: 'decimal', precision: 3, scale: 2, nullable: true }
    ],
    indexes: [
      'member_id', 'channel', 'direction', 'status', 'created_at', 'provider_id'
    ]
  },
  ...
  members: {
    fields: [
      ...
      { name: 'lifetime_value', type: 'decimal', precision: 10, scale: 2, default: 0 },
      ...
      { name: 'billing_cycle', type: 'enum', values: ['monthly', 'yearly', 'lifetime'], nullable: true },
    ...
    indexes: [
      'email', 'phone', 'status', 'membership_type', 'created_at'
    ]
  },
  ...
  member_benefits: {
    fields: [
      ...
      { name: 'benefit_type', type: 'enum', values: ['discount', 'access', 'service', 'product', 'support'] },
      { name: 'benefit_category', type: 'enum', values: ['billing', 'shipping', 'support', 'exclusive', 'partner'] },
    ...
    indexes: [
      'is_active', 'benefit_type', 'benefit_category', 'sort_order'
    ]
  },
...
const setupInstructions = () => {
  ...
  Object.keys(XANO_TABLES).forEach((tableName, index) => {
    ...
    console.log(`   ${index + 1}. Create table: "${tableName}" (${table.fields.length} fields)`);
  });
  console.log('');
...
const generateSQLFile = () => {
  ...
  Object.entries(XANO_TABLES).forEach(([tableName, table]) => {
    ...
    table.fields.forEach((field, index) => {
      ...
      if (field.precision) sql += `(${field.precision}${field.scale ? `,${field.scale}` : ''})`;
...
const runSetup = () => {
  ...
  console.log('🎉 Setup completed! Follow the instructions above to configure your Xano workspace.');
  console.log('💡 Pro tip: Copy the SQL from XANO_SETUP_GUIDE.sql to quickly create tables.');
};
