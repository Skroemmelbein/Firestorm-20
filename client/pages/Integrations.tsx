          ...
          <TabsContent value="xano" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Connect to your Xano backend instance and database
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="xano-url">Instance URL</Label>
                  <Input
                    id="xano-url"
                    placeholder="https://your-instance.xano.io"
                    value={xanoConfig.instanceUrl}
                    onChange={(e) => setXanoConfig(prev => ({ ...prev, instanceUrl: e.target.value }))}
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="xano-api-key">API Key</Label>
                  <div className="relative">
                    <Input
                      id="xano-api-key"
                      type={showXanoApiKey ? "text" : "password"}
                      placeholder="Your Xano API key"
                      value={xanoConfig.apiKey}
                      onChange={(e) => setXanoConfig(prev => ({ ...prev, apiKey: e.target.value }))}
                      className="pr-20"
                    />
                    <div className="absolute right-2 top-2.5 flex gap-1">
                      <Button
                        ...
                        onClick={() => setShowXanoApiKey(!showXanoApiKey)}
                      >
                        {showXanoApiKey ? <EyeOff className="w-3 h-3" /> : <Eye className="w-3 h-3" />}
                      </Button>
                      <Button
                        variant="ghost"
                        size="sm"
                        className="h-6 w-6 p-0"
                        onClick={() => copyToClipboard(xanoConfig.apiKey)}
                      >
                ...
                <div className="space-y-2">
                  <Label htmlFor="xano-db-id">Database ID</Label>
                  <Input
                    id="xano-db-id"
                    placeholder="Your database ID"
                    value={xanoConfig.databaseId}
                    onChange={(e) => setXanoConfig(prev => ({ ...prev, databaseId: e.target.value }))}
                  />
                </div>

                <div className="flex gap-2">
                  <Button onClick={testXanoConnection} disabled={testing.xano} variant="outline">
                    {testing.xano ? 'Testing...' : 'Test Connection'}
                  </Button>
                </div>
                ...
                {xanoStatus.error && (
                  <Alert variant="destructive">
                    ...
                    <AlertDescription>{xanoStatus.error}</AlertDescription>
                  </Alert>
                )}
              </CardContent>
            </Card>
