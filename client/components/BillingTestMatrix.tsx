...
export default function BillingTestMatrix() {
  const [testScenarios, setTestScenarios] = useState<TestScenario[]>([
    // CIT Tests
    {
      ...
      category: "cit",
      description:
        "Customer initiated transaction with successful vault token creation",
      expected_result:
        "Transaction approved, vault token created, subscription activated",
      test_data: {
        card: {
    ...
    },
    {
      ...
      expected_result:
        "Transaction declined, no vault token created, appropriate error message",
      test_data: {
        card: {
    ...
    // MIT Tests
    {
      ...
      expected_result:
        "Transaction approved, next billing date updated, retry count reset",
      test_data: {
        subscription_id: 123,
    ...
    },
    {
      ...
      expected_result:
        "Transaction declined, retry scheduled, appropriate retry logic triggered",
      test_data: {
        subscription_id: 124,
        expected_decline: "51",
      },
    ...
    // Decline Scenario Tests
    {
      ...
      expected_result:
        "Decline detected, card update required flag set, no retry scheduled",
      test_data: {
        response_code: "54",
    ...
    },
    {
      ...
      expected_result:
        "Retry scheduled with descriptor variation, proper backoff timing",
      test_data: {
        response_code: "05",
        expected_retries: 3,
    ...
    },
    {
      ...
      expected_result:
        "Subscription marked as past_due, no further retries scheduled",
      test_data: {
        current_retries: 3,
        max_retries: 3,
      },
    ...
    },
    {
      ...
      expected_result:
        "Descriptor is 22 characters or less, no special characters",
      test_data: {
        test_descriptors: [
          "ECELONX Subscription",
          "ECELONX Sub *Renew",
          "ECELONX Monthly",
        ],
      },
    ...
    // Tokenization Tests
    {
      ...
      expected_result:
        "Network tokens created for Visa/MC cards, improved approval rates",
      test_data: {
  ...
  const runSingleTest = async (testId: string) => {
    ...
    try {
      ...
      console.log(
        `${success ? "✅" : "❌"} Test ${test.name} ${success ? "passed" : "failed"}`,
      );
    } catch (error: any) {
  ...
  const getOverallStats = () => {
    ...
    const passed = filteredScenarios.filter(
      (s) => s.status === "passed",
    ).length;
    const failed = filteredScenarios.filter(
      (s) => s.status === "failed",
    ).length;
    const running = filteredScenarios.filter(
      (s) => s.status === "running",
    ).length;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">
            Comprehensive QA testing for high-approval billing stack
          </p>
        </div>
                  ...
                  <div className="text-sm space-y-1">
                    ...
                    {scenario.last_run && (
                      <div className="flex items-center gap-4 text-gray-500">
                        <span>
                          Last run:{" "}
                          {new Date(scenario.last_run).toLocaleString()}
                        </span>
                        {scenario.duration && (
