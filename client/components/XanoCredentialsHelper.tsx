...
export default function XanoCredentialsHelper() {
  ...
  const testConnection = async () => {
    ...
    try {
      const response = await httpRequest(`${window.location.origin}/api/real/test/xano`, {
        method: "POST",
  ...
  const saveConfig = async () => {
    try {
      const response = await httpRequest(`${window.location.origin}/api/integrations/config`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      <Card className="glass-card corp-shadow border-blue-200">
        ...
        <CardContent className="space-y-4">
          ...
          <Alert>
            ...
            <AlertDescription>
              <strong>Need help?</strong> If you can't find these details,
              create a new Xano workspace and I'll help you set it up with the
              exact tables we need.
            </AlertDescription>
          </Alert>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) =>
                setConfig((prev) => ({ ...prev, instanceUrl: e.target.value }))
              }
            />
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) =>
                setConfig((prev) => ({ ...prev, databaseId: e.target.value }))
              }
            />
            <p className="text-xs text-muted-foreground">
          ...
          <div className="flex gap-2 pt-4">
            ...
            <Button
              ...
              onClick={saveConfig}
              disabled={
                !config.instanceUrl || !config.apiKey || !config.databaseId
              }
            >
              Save Configuration
            </Button>
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm">
                  Create the required database tables in Xano
                </span>
              </div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm">
                  Go to Member Portal to see your benefits system
                </span>
              </div>
              <div className="flex items-center gap-2">
                <Badge
