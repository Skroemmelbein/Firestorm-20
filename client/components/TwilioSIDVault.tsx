...
interface TwilioSID {
  ...
  name: string;
  sid: string;
  type: 'phone' | 'service' | 'flow' | 'agent' | 'conversation' | 'workspace' | 'activity' | 'other';
  description?: string;
  dateAdded: string;
  lastUsed?: string;
...
const SID_TYPES = [
  ...
  { value: 'service', label: 'Messaging Service', icon: MessageSquare, color: 'bg-green-500' },
  { value: 'flow', label: 'Studio Flow', icon: Brain, color: 'bg-purple-500' },
  { value: 'agent', label: 'RCS Agent', icon: Network, color: 'bg-orange-500' },
  { value: 'conversation', label: 'Conversation', icon: Users, color: 'bg-pink-500' },
  { value: 'workspace', label: 'TaskRouter Workspace', icon: Settings, color: 'bg-indigo-500' },
  { value: 'activity', label: 'TaskRouter Activity', icon: Settings, color: 'bg-cyan-500' },
...
export default function TwilioSIDVault() {
  ...
  const discoverSIDs = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const newSids = discovered.filter((d: any) => !existingSids.includes(d.sid));
  ...
  const filteredSids = sids.filter(sid => {
    const matchesSearch = sid.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const getTypeInfo = (type: string) => {
    return SID_TYPES.find(t => t.value === type) || SID_TYPES[SID_TYPES.length - 1];
  };
    ...
    <div className="w-full max-w-6xl mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Manage and organize your Twilio Service Identifiers</p>
        </div>
        ...
        <TabsContent value="manage" className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {filteredSids.map((sid) => {
              ...
              return (
                <Card key={sid.id} className="border-2 border-gray-200 hover:border-blue-300 transition-colors">
                  <CardContent className="p-4">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <h3 className="font-semibold text-gray-800">{sid.name}</h3>
                          <Badge variant="outline" className="text-xs">
                            {typeInfo.label}
                          </Badge>
                        </div>
                    ...
                    <div className="space-y-2">
                      ...
                      {sid.description && (
                        <p className="text-sm text-gray-600">{sid.description}</p>
                      )}
                      <div className="flex justify-between text-xs text-gray-500">
                        <span>Added: {new Date(sid.dateAdded).toLocaleDateString()}</span>
                        <Badge 
                          variant={sid.status === 'active' ? 'default' : 'secondary'}
                          className="text-xs"
                        >
                          {sid.status}
                        </Badge>
          ...
          {filteredSids.length === 0 && (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-600 mb-2">No SIDs found</h3>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    value={newSid.name || ''}
                    onChange={(e) => setNewSid(prev => ({ ...prev, name: e.target.value }))}
                ...
                <div>
                  ...
                  <select
                    value={newSid.type}
                    onChange={(e) => setNewSid(prev => ({ ...prev, type: e.target.value as TwilioSID['type'] }))}
              ...
              <div>
                ...
                <Input
                  value={newSid.sid || ''}
                  onChange={(e) => setNewSid(prev => ({ ...prev, sid: e.target.value }))}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setNewSid(prev => ({ ...prev, description: e.target.value }))}
          ...
          <Card className="border-2 border-orange-200">
            <CardContent className="p-4">
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <h4 className="font-medium text-orange-800">Finding Your Twilio SIDs</h4>
                  <p className="text-sm text-orange-700 mt-1">
                    You can find your SIDs in the Twilio Console under each service section:
                  </p>
                  <ul className="text-sm text-orange-700 mt-2 space-y-1">
                    <li>• Phone Numbers: Console → Phone Numbers → Manage</li>
                    <li>• Messaging Services: Console → Messaging → Services</li>
                    <li>• Studio Flows: Console → Studio → Flows</li>
                    <li>• RCS Agents: Console → Messaging → Rich Communication Services</li>
                  </ul>
                </div>
              </div>
            </CardContent>
            ...
            <Card className="border-2 border-green-200">
              ...
              <CardContent>
                <p className="text-gray-600 mb-4">
                  Export all your stored SIDs to a JSON file for backup or sharing.
                </p>
                <Button
                  onClick={exportSIDs}
