...
export default function CheckoutForm({ plans = [], onSuccess, onError }: CheckoutFormProps) {
  ...
  const validateForm = (): boolean => {
    ...
    else if (formData.ccnumber.replace(/\s/g, '').length < 13) newErrors.ccnumber = 'Invalid card number';

    if (!formData.ccexp) newErrors.ccexp = 'Expiry date is required';
    else if (!/^\d{2}\/\d{2}$/.test(formData.ccexp)) newErrors.ccexp = 'Invalid expiry format (MM/YY)';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('🚀 Processing CIT initial charge...', {
        ...
        amount: selectedPlan ? formatAmount(selectedPlan.amount_cents) : 'Unknown'
      });
        ...
        <CardContent className="space-y-4">
          <div className="bg-green-50 p-4 rounded-lg space-y-2">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium">{selectedPlan ? formatAmount(selectedPlan.amount_cents) : 'N/A'}</span>
            </div>
            <div className="flex justify-between text-sm">
            ...
            {result.transaction?.transaction_id && (
              <div className="flex justify-between text-sm">
                ...
                <span className="font-mono text-xs">{result.transaction.transaction_id}</span>
              </div>
            )}
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-4 p-4 bg-gray-50 rounded-lg">
            ...
            <div className="space-y-2">
              ...
              />
              {errors.ccnumber && <p className="text-sm text-red-500">{errors.ccnumber}</p>}
            </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                />
                {errors.ccexp && <p className="text-sm text-red-500">{errors.ccexp}</p>}
              </div>
            ...
            <div className="bg-blue-50 p-4 rounded-lg">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-blue-900">
                  {formatAmount(selectedPlan.amount_cents)}/{selectedPlan.interval}
                </span>
              </div>
          ...
          >
            {isProcessing ? (
              ...
              <>
                ...
                Pay {selectedPlan ? formatAmount(selectedPlan.amount_cents) : ''}
              </>
            )}
          </Button>
