...
const NODE_TYPES = {
  trigger: {
    ...
    color: '#10B981',
    bgColor: '#10B98110',
    options: ['Form Submission', 'Email Signup', 'Purchase', 'Page Visit', 'Custom Event']
  },
  action: {
    icon: Send,
    color: '#FF6A00',
    bgColor: '#FF6A0010',
    options: ['Send Email', 'Send SMS', 'Add to List', 'Update Contact', 'Send Webhook']
  },
  condition: {
    icon: Split,
    color: '#8B5CF6',
    bgColor: '#8B5CF610',
    options: ['Contact Property', 'Engagement Score', 'Purchase History', 'Location', 'Device Type']
  },
  delay: {
    icon: Timer,
    color: '#F59E0B',
    bgColor: '#F59E0B10',
    options: ['Wait Duration', 'Wait Until Date', 'Wait for Event', 'Business Hours Only']
  },
  split: {
    icon: Target,
    color: '#EF4444',
...
export default function CampaignBuilder() {
  ...
  const [campaigns, setCampaigns] = useState<Campaign[]>([
    {
      ...
      status: 'active',
      nodes: [],
      stats: { sent: 1250, delivered: 1200, opened: 720, clicked: 180, converted: 45 },
    ...
    },
    {
      ...
      status: 'active',
      nodes: [],
      stats: { sent: 850, delivered: 820, opened: 410, clicked: 82, converted: 28 },
  ...
  const handleNodeDrop = useCallback((e: React.DragEvent) => {
    ...
    const newNode: CampaignNode = {
      ...
      title: NODE_TYPES[draggedNodeType as keyof typeof NODE_TYPES].options[0],
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {campaigns.map((campaign) => {
          ...
          const conversionRate = campaign.stats.sent > 0 
            ? ((campaign.stats.converted / campaign.stats.sent) * 100).toFixed(1)
            : '0.0';

          return (
            <Card key={campaign.id} className="f10-card hover:border-[#FF6A00]/50 transition-all cursor-pointer">
              ...
              <CardContent>
                <div className="grid grid-cols-3 gap-3 mb-3">
                  <div className="text-center">
                    <div className="text-lg font-bold text-[#FF6A00]">{campaign.stats.sent.toLocaleString()}</div>
                    <div className="text-xs text-[#737373]">SENT</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-bold text-[#10B981]">{campaign.stats.opened.toLocaleString()}</div>
                    <div className="text-xs text-[#737373]">OPENED</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-bold text-[#FFD700]">{conversionRate}%</div>
      ...
      {selectedCampaign && selectedCampaign.nodes.length === 0 && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center text-[#737373]">
            ...
            <p className="text-sm">Drag elements from the palette to create your automation flow</p>
          </div>
        </div>
      )}
    </div>
  ...
  const renderNewCampaignForm = () => (
    <div className="max-w-md mx-auto space-y-4">
      <h3 className="text-xl font-bold text-white text-center">Create New Campaign</h3>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <Select
            ...
            onValueChange={(value) => setNewCampaignForm(prev => ({ ...prev, trigger: value }))}
          >
