...
const SMS_CATEGORIES = [
  ...
  },
  {
    ...
    icon: ShoppingCart,
    color: "#EF4444",
    description: "Recover abandoned purchases",
  },
  {
    id: "reminder",
    name: "Reminders",
    icon: Clock,
    color: "#F59E0B",
    description: "Appointment and deadline alerts",
  },
  {
    id: "transactional",
    name: "Transactional",
...
const PERSONALIZATION_VARIABLES: PersonalizationVariable[] = [
  ...
  },
  {
    key: "{location}",
    name: "Location",
    description: "Store or event location",
...
const FIRESTORM_SMS_TEMPLATES: Partial<SMSTemplate>[] = [
  {
    ...
    category: "welcome",
    message:
      "Welcome to FIRESTORM {firstName}! 🚀 Your marketing automation journey starts now. Get ready for explosive results! Reply HELP for support.",
  ...
  },
  {
    ...
    message:
      "🔥 FLASH SALE {firstName}! {discount} OFF everything! Use code {discountCode} - expires in 24hrs! Shop now: [link] Reply STOP to opt out",
  ...
  },
  {
    ...
    message:
      "Hey {firstName}, you left {productName} in your cart! 🛒 Complete your order now and save {discount}! [checkout-link] Questions? Reply here!",
  ...
  },
  {
    ...
    category: "reminder",
    message:
      "Hi {firstName}! Reminder: Your appointment is tomorrow at {appointmentTime} at {location}. Reply C to confirm or R to reschedule.",
  ...
  },
  {
    ...
    message:
      "Order confirmed! 🎉 Thanks {firstName}! Order {orderNumber} for {orderTotal} is being processed. Track: [link] Need help? Reply here!",
  ...
  },
  {
    ...
    category: "follow-up",
    message:
      "Hi {firstName}! How was your experience with {productName}? ⭐ Share a quick review: [review-link] Your feedback helps us improve!",
...
export default function SMSTemplateLibrary() {
  const [templates, setTemplates] = useState<SMSTemplate[]>(
    FIRESTORM_SMS_TEMPLATES.map((t) => ({
      ...
      stats: {
        ...
        responseRate: Math.floor(Math.random() * 15),
        conversionRate: Math.floor(Math.random() * 8),
        optOuts: Math.floor(Math.random() * 5),
      },
  ...
  const getCharacterWarning = (count: number) => {
    ...
    return {
      color: "#EF4444",
      message: `${Math.ceil(count / 160)} SMS messages`,
    };
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[#b3b3b3]">
            Create, manage, and deploy SMS campaigns with personalization
          </p>
        </div>
        <Button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredTemplates.map((template) => {
          const category = SMS_CATEGORIES.find(
              ...
              <CardContent>
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span style={{ color: charWarning.color }}>
                      {charWarning.message}
                    </span>
                  </div>
            ...
            <div>
              ...
              <Textarea
                ...
                onChange={(e) => {
                  ...
                  setPreviewMessage(
                    personalizeMessage(e.target.value, testData),
                  );
                }}
              ...
              <div className="flex items-center justify-between mt-2 text-xs">
                <span className="text-[#737373]">
                  {newTemplate.message.length} characters
                </span>
                <span
                  style={{
                    color: getCharacterWarning(newTemplate.message.length)
                      .color,
                  }}
                >
            ...
            <CardContent className="space-y-3">
              {Object.entries(testData).map(([key, value]) => (
                <div key={key} className="flex gap-2">
                  <Label className="text-[#b3b3b3] text-xs w-20 capitalize">
                    {key}:
                  </Label>
                  <Input
                    value={value}
                    onChange={(e) => {
                      ...
                      setPreviewMessage(
                        personalizeMessage(newTemplate.message, {
                          ...testData,
                          [key]: e.target.value,
                        }),
                      );
            ...
            <CardContent>
              <div className="bg-[#0a0a0a] border border-[#333333] rounded p-3">
                <div className="text-sm text-white whitespace-pre-wrap">
                  {personalizeMessage(newTemplate.message, testData) ||
                    "Type a message to see preview..."}
                </div>
              </div>
              <div className="mt-2 text-xs text-[#737373]">
                Preview with test data •{" "}
                {personalizeMessage(newTemplate.message, testData).length} chars
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="f10-card">
          <CardHeader>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                {SMS_CATEGORIES.find((c) => c.id === selectedTemplate.category)
                  ?.icon &&
                  React.createElement(
                    SMS_CATEGORIES.find(
          ...
          <CardContent className="space-y-6">
            ...
            <div>
              ...
              <div className="flex items-center justify-between mt-2 text-xs">
                <span className="text-[#737373]">
                  {selectedTemplate.characterCount} characters
                </span>
                <span
                  style={{
                    color: getCharacterWarning(selectedTemplate.characterCount)
                      .color,
                  }}
                >
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {selectedTemplate.variables.map((variable) => (
                    <Badge
                      key={variable}
                      className="bg-[#FF6A00]/20 text-[#FF6A00] border-[#FF6A00]/40"
                    >
                      {"{" + variable + "}"}
                    </Badge>
                  ))}
