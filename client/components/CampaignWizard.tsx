...
export default function CampaignWizard() {
  ...
  const tools = [
    { id: 'sms', name: 'SMS', icon: MessageSquare, desc: 'Text messaging', active: true },
    { id: 'mms', name: 'MMS', icon: Image, desc: 'Multimedia messaging', active: true },
    { id: 'rcs', name: 'RCS', icon: MessageCircle, desc: 'Rich messaging', active: false },
    { id: 'email', name: 'SendGrid Email', icon: Mail, desc: 'Email campaigns', active: true },
    { id: 'vmail', name: 'Voicemail Drop', icon: Phone, desc: 'Voice messages', active: false }
  ];

  const campaignTypes = [
    { id: 'current', name: 'Current Clients', desc: 'Active customers', count: 1247 },
    { id: 'cancelled', name: 'Cancelled Clients', desc: 'Former customers', count: 892 },
    { id: 'billing', name: 'Billing Reminders', desc: 'Payment notifications', count: 156 },
    { id: 'new_list', name: 'New Lead List', desc: 'Fresh prospects', count: 0 },
    { id: 'ppc', name: 'PPC Leads', desc: 'Paid advertising leads', count: 523 },
    { id: 'referral', name: 'Referral Program', desc: 'Referred prospects', count: 78 },
    { id: 'reactivation', name: 'Re-activation', desc: 'Win-back campaign', count: 445 },
    { id: 'upsell', name: 'Upsell Campaign', desc: 'Upgrade offers', count: 234 }
  ];
  ...
  const calculateEstimates = () => {
    const selectedCampaign = campaignTypes.find(c => c.id === config.campaignType);
    ...
    setConfig(prev => ({ ...prev, estimatedReach: reach, estimatedCost: cost }));
  };
          ...
          {currentStep === 1 && (
            <div className="space-y-6">
              <div className="text-center">
                <h2 className="text-2xl font-bold">Choose Communication Tools</h2>
                <p className="text-muted-foreground">Select the channels for your campaign</p>
              </div>
              ...
              {config.tools.length > 0 && (
                <Alert>
                  ...
                  <AlertDescription>
                    Selected {config.tools.length} tool{config.tools.length > 1 ? 's' : ''}: {config.tools.map(t => tools.find(tool => tool.id === t)?.name).join(', ')}
                  </AlertDescription>
                </Alert>
              )}
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {campaignTypes.map((type) => (
                  <div
                    key={type.id}
                    onClick={() => setConfig(prev => ({ ...prev, campaignType: type.id }))}
                    className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <div className="text-2xl font-bold text-blue-600">{type.count.toLocaleString()}</div>
          ...
          {currentStep === 3 && (
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-muted-foreground">Refine your audience selection</p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {audiences.map((audience) => (
                  <div
                    key={audience.id}
                    onClick={() => setConfig(prev => ({ ...prev, audience: audience.id }))}
                    className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${
                  ...
                  >
                    ...
                    <div className="text-sm text-muted-foreground">{audience.desc}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {currentStep === 4 && (
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-muted-foreground">Configure sending rate and phone numbers</p>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <Slider
                    ...
                    onValueChange={(value) => setConfig(prev => ({ ...prev, throughputRate: value[0] }))}
                    max={100}
                    min={1}
                    step={1}
                    className="w-full"
                  ...
                  <div className="text-sm text-muted-foreground">
                    Higher rates may trigger carrier filtering. Recommended: 10-20/min
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <Select value={config.fromNumber} onValueChange={(value) => setConfig(prev => ({ ...prev, fromNumber: value }))}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
          ...
          {currentStep === 5 && (
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-muted-foreground">Schedule your campaign execution</p>
              </div>
          ...
          {currentStep === 6 && (
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-muted-foreground">Confirm your campaign settings</p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div className="flex justify-between">
                      <span>Type:</span>
                      <span>{campaignTypes.find(c => c.id === config.campaignType)?.name}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Audience:</span>
                      <span>{audiences.find(a => a.id === config.audience)?.name}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Rate:</span>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    <div className="flex justify-between">
                      ...
                      <span className="text-2xl font-bold text-blue-600">{config.estimatedReach.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Estimated Cost:</span>
                      <span className="text-2xl font-bold text-green-600">${config.estimatedCost.toFixed(2)}</span>
                    </div>
                    <div className="text-sm text-muted-foreground">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="bg-gray-100 p-4 rounded-lg">
                    <div className="text-sm font-medium mb-2">From: {config.fromNumber}</div>
