...
const NMILegacyVaultSchema = z.object({
  ...
  legacy_vault_records: z.array(z.object({
...
interface VaultMigrationEngine {
  ...
  assessRisk(vault: any): { score: number; factors: string[]; recommendation: string };
}

class NMILegacyMigrationEngine implements VaultMigrationEngine {
  
  async processLegacyVault(vault: any): Promise<z.infer<typeof TokenMappingSchema>> {
  ...
  validatePaymentMethod(vault: any): boolean {
    ...
    if (!vault.cc_number_masked || vault.cc_number_masked.length < 4) return false;
  ...
  private hasRiskKeywords(notes: string): boolean {
    const riskKeywords = [
      "chargeback", "dispute", "fraud", "suspicious", "declined", 
      "failed", "risk", "block", "hold", "investigation"
    ];
...
export const startLegacyVaultMigration: RequestHandler = async (req, res) => {
  try {
    ...
    const { legacy_vault_records, import_batch_id, total_records } = validation.data;
...
async function processVaultMigration(data: z.infer<typeof NMILegacyVaultSchema>) {
  ...
  try {
    for (let i = 0; i < legacy_vault_records.length; i += BATCH_SIZE) {
      ...
      for (const vaultRecord of batch) {
        try {
          const mappingResult = await migrationEngine.processLegacyVault(vaultRecord);
          results.push(mappingResult);
          
          // Log successful mapping
          if (mappingResult.mapping_status === "MAPPED") {
            console.log(`✅ Mapped vault ${vaultRecord.customer_vault_id} -> ${mappingResult.new_vault_id}`);
          } else {
            console.log(`⚠️  Vault ${vaultRecord.customer_vault_id} needs attention: ${mappingResult.mapping_status}`);
          }
          
        } catch (error: any) {
          console.error(`❌ Failed to process vault ${vaultRecord.customer_vault_id}:`, error.message);
...
function generateMigrationSummary(results: z.infer<typeof TokenMappingSchema>[]) {
  const summary = {
    total_processed: results.length,
    successful_mappings: results.filter(r => r.mapping_status === "MAPPED").length,
    needs_validation: results.filter(r => r.mapping_status === "NEEDS_VALIDATION").length,
    failed_mappings: results.filter(r => r.mapping_status === "FAILED").length,
    duplicates_found: results.filter(r => r.mapping_status === "DUPLICATE").length,
    high_risk_accounts: results.filter(r => r.risk_assessment.score >= 70).length,
    avg_risk_score: Math.round(results.reduce((sum, r) => sum + r.risk_assessment.score, 0) / results.length)
  };
