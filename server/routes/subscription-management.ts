...
router.post('/create', async (req, res) => {
  try {
    const { customer, paymentMethod, subscription }: CreateSubscriptionRequest = req.body;
    ...
    if (paymentMethod.type === 'credit_card') {
      ...
      customerParams.append('ccexp', `${paymentMethod.expiryMonth}${paymentMethod.expiryYear}`);
      customerParams.append('cvv', paymentMethod.cvv!);
    } else if (paymentMethod.type === 'bank_account') {
      customerParams.append('checkname', `${customer.firstName} ${customer.lastName}`);
    ...
    if (customerParams2.get('response') !== '1') {
      throw new Error(`Customer creation failed: ${customerParams2.get('responsetext')}`);
    }
    ...
    const startDate = subscription.startDate || new Date().toISOString().split('T')[0].replace(/-/g, '');
    ...
    if (subscription.trialDays && subscription.trialDays > 0) {
      ...
      subscriptionParams.set('start_date', trialEndDate.toISOString().split('T')[0].replace(/-/g, ''));
    }
    ...
    if (subscriptionParams2.get('response') !== '1') {
      throw new Error(`Subscription creation failed: ${subscriptionParams2.get('responsetext')}`);
    }
    ...
    const xanoMember = await xanoAPI.createRecord('members', {
      ...
      last_name: customer.lastName,
      status: 'active',
      membership_type: subscription.planId.includes('premium') ? 'premium' : 'basic',
    ...
    const xanoSubscription = await xanoAPI.createRecord('subscriptions', {
      ...
      currency: 'USD',
      billing_cycle: subscription.frequency,
      next_billing_date: new Date(Date.now() + getFrequencyDays(subscription.frequency) * 24 * 60 * 60 * 1000).toISOString(),
...
router.patch('/:id/status', async (req, res) => {
  try {
    ...
    const updatedSubscription = await xanoAPI.updateRecord('subscriptions', id, updateData);
  ...
  try {
    ...
    const analytics = {
      ...
      yearlyRevenue: subscriptions
        .filter((s: any) => s.status === 'active' && s.billing_cycle === 'yearly')
        .reduce((sum: number, s: any) => sum + parseFloat(s.amount || 0), 0)
    };
