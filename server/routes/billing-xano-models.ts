...
const BILLING_STACK_TABLES = [
  ...
  
  {
    ...
    columns: [
      { name: 'id', type: 'integer', auto_increment: true, primary_key: true },
      { name: 'user_id', type: 'integer', foreign_key: 'users.id', indexed: true },
  ...
  
  {
    ...
    columns: [
      { name: 'id', type: 'integer', auto_increment: true, primary_key: true },
      { name: 'subscription_id', type: 'integer', foreign_key: 'subscriptions.id', indexed: true },
      { name: 'user_id', type: 'integer', foreign_key: 'users.id', indexed: true },
  ...
  
  {
    ...
    columns: [
      ...
      { name: 'related_transaction_id', type: 'integer', foreign_key: 'transactions.id' },
      { name: 'related_subscription_id', type: 'integer', foreign_key: 'subscriptions.id' },
  ...
  
  {
    ...
    columns: [
      ...
      { name: 'status', type: 'enum', values: ['pending', 'executed', 'skipped'] },
  ...
  
  {
    ...
    columns: [
      ...
      { name: 'approval_rate_percent', type: 'decimal', precision: 5, scale: 2 },
      ...
      { name: 'retry_success_rate_percent', type: 'decimal', precision: 5, scale: 2 },
  ...
  
  {
    ...
    columns: [
      ...
      { name: 'card_brand', type: 'text' }, // visa, mastercard, etc.
      { name: 'retry_stage', type: 'enum', values: ['initial', 'retry_1', 'retry_2', 'retry_3'] },
      { name: 'created_at', type: 'timestamp' }
    ]
  }
];
  ...
  try {
    ...
    for (const table of BILLING_STACK_TABLES) {
      try {
        ...
        console.log(`✅ Table ${table.name} created with ${table.columns.length} columns`);
  ...
  try {
    ...
    for (const table of BILLING_STACK_TABLES) {
      try {
        ...
        if (table.name === 'transactions') health.summary.totalTransactions = records.length;
        if (table.name === 'subscriptions') {
          health.summary.activeSubscriptions = records.filter((s: any) => s.status === 'active').length;
        }
      } catch (error) {
        health.tables[table.name] = {
          exists: false,
    ...
    try {
      ...
      health.summary.approvalRate = transactions.length > 0 ? 
        Math.round((approved / transactions.length) * 100) : 0;
    } catch (e) {
