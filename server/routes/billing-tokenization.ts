...
class TokenizationService {
  ...
  async enableNetworkTokenization(customerVaultId: string): Promise<any> {
    try {
      console.log(
        `🔒 Enabling network tokenization for vault: ${customerVaultId}`,
      );
      ...
      if (resultParams.get("response") === "1") {
        ...
        console.log(
          `✅ Network tokenization enabled for vault: ${customerVaultId}`,
        );
        ...
        throw new Error(
          resultParams.get("responsetext") || "Network tokenization failed",
        );
      }
    } catch (error: any) {
      console.error(
        `❌ Network tokenization failed for vault ${customerVaultId}:`,
        error,
      );
      throw error;
    }
  ...
  async requestCardUpdate(customerVaultId: string): Promise<any> {
    try {
      ...
      console.error(
        `❌ Card update failed for vault ${customerVaultId}:`,
        error,
      );
      throw error;
    }
  ...
  isEligibleForNetworkTokens(cardBrand: string, cardBin: string): boolean {
    ...
    return (
      supportedBrands.includes(cardBrand.toLowerCase()) &&
...
router.post("/enable-network-tokens", async (req, res) => {
  try {
    const { customer_vault_id, subscription_id }: NetworkTokenRequest =
      req.body;
    ...
    if (
      !tokenizationService.isEligibleForNetworkTokens(
    ...
    console.log(
      `✅ Network tokenization enabled for subscription ${subscription_id}`,
    );
  ...
  try {
    ...
    if (subscription_ids && subscription_ids.length > 0) {
      ...
      subscriptions = allSubscriptions.filter((sub: any) => {
        ...
        return (
          force_update ||
          tokenizationService.isCardNearExpiration(
            sub.card_exp_month,
            sub.card_exp_year,
          ) ||
          sub.status === "past_due"
    ...
    console.log(
      `📋 Found ${subscriptions.length} subscriptions for card update`,
    );
    ...
    for (const subscription of subscriptions) {
      ...
      try {
        console.log(
          `🔄 Processing card update for subscription ${subscription.id}...`,
        );

        // Request card update
        const updateResult = await tokenizationService.requestCardUpdate(
        ...
        if (updateResult.success && updateResult.updated_card) {
          ...
          await getXanoClient().updateRecord("subscriptions", subscription.id, {
            ...
            card_exp_month:
              parseInt(updateResult.updated_card.exp_month) ||
              subscription.card_exp_month,
            card_exp_year:
              parseInt(`20${updateResult.updated_card.exp_year}`) ||
              subscription.card_exp_year,
            card_brand:
              updateResult.updated_card.brand || subscription.card_brand,
            auto_card_updater_last_update: new Date().toISOString(),
            updated_at: new Date().toISOString(),
          });
        ...
        console.error(
          `❌ Error updating card for subscription ${subscription.id}:`,
          error,
        );
        results.push({
          subscription_id: subscription.id,
    ...
    console.log(
      `✅ Card updater complete: ${updated} updated, ${noUpdate} no update needed, ${errors} errors`,
    );
...
router.get("/tokenization-status", async (req, res) => {
  try {
    ...
    const status = {
      ...
      network_tokens_enabled: subscriptions.filter(
        (s: any) => s.network_token_enabled,
      ).length,
      auto_updater_enabled: subscriptions.filter(
        (s: any) => s.auto_card_updater_enabled,
      ).length,
      ...
      by_card_brand: {
        ...
        mastercard: subscriptions.filter(
          (s: any) => s.card_brand === "mastercard",
        ).length,
        amex: subscriptions.filter((s: any) => s.card_brand === "amex").length,
        discover: subscriptions.filter((s: any) => s.card_brand === "discover")
          .length,
      },
      last_updater_run:
        subscriptions
          ...
          .sort(
            (a: any, b: any) =>
              new Date(b.auto_card_updater_last_update).getTime() -
              new Date(a.auto_card_updater_last_update).getTime(),
          )[0]?.auto_card_updater_last_update || null,
    };
...
router.post("/batch-enable-network-tokens", async (req, res) => {
  try {
    ...
    const eligibleSubscriptions = subscriptions.filter((sub: any) => {
      ...
      return tokenizationService.isEligibleForNetworkTokens(
        sub.card_brand,
        cardBin,
      );
    });
    ...
    console.log(
      `📋 Found ${eligibleSubscriptions.length} eligible subscriptions for network tokenization`,
    );
    ...
    for (const subscription of eligibleSubscriptions) {
      try {
        const tokenResult = await tokenizationService.enableNetworkTokenization(
          subscription.nmi_customer_vault_id,
        );
  ...
  try {
    ...
    const expiringCards = subscriptions.filter((sub: any) =>
      tokenizationService.isCardNearExpiration(
        sub.card_exp_month,
        sub.card_exp_year,
        parseInt(months_ahead as string),
      ),
    );
    ...
    expiringCards.forEach((sub: any) => {
      ...
      report.by_brand[sub.card_brand] =
        (report.by_brand[sub.card_brand] || 0) + 1;
    });
