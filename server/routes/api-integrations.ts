...
const NMI_CONFIG = {
  ...
  recurringVaultUrl: process.env.NMI_RECURRING_VAULT_URL || 'https://secure.nmi.com/api/recurring.php'
};
...
class XanoAPI {
  ...
  async createRecurringSubscription(subscriptionData: any) {
    return this.makeRequest('/recurring-subscriptions', 'POST', subscriptionData);
  }
}
...
class TwilioAPI {
  ...
  async sendSMS(to: string, message: string, campaignId?: string) {
    const response = await fetch(`https://api.twilio.com/2010-04-01/Accounts/${this.client.accountSid}/Messages.json`, {
      method: 'POST',
      headers: {
  ...
  async makeCall(to: string, script: string, campaignId?: string) {
    const response = await fetch(`https://api.twilio.com/2010-04-01/Accounts/${this.client.accountSid}/Calls.json`, {
      method: 'POST',
      headers: {
        'Authorization': `Basic ${Buffer.from(`${this.client.accountSid}:${this.client.authToken}`).toString('base64')}`,
        'Content-Type': 'application/x-www-form-urlencoded'
...
class NMIAPI {
  async createRecurringSubscription(customerData: any, subscriptionData: any) {
    const params = new URLSearchParams({
      ...
      start_date: subscriptionData.startDate || new Date().toISOString().split('T')[0],
    ...
    if (responseCode === '1') {
      ...
      throw new Error(`NMI recurring subscription failed: ${resultParams.get('responsetext')}`);
    }
  }
  ...
  async pauseSubscription(subscriptionId: string) {
    return this.updateRecurringSubscription(subscriptionId, { recurring: 'pause_subscription' });
  }

  async resumeSubscription(subscriptionId: string) {
    return this.updateRecurringSubscription(subscriptionId, { recurring: 'resume_subscription' });
  }

  async cancelSubscription(subscriptionId: string) {
    return this.updateRecurringSubscription(subscriptionId, { recurring: 'delete_subscription' });
  }
}
...
router.post('/subscriptions/recurring', async (req, res) => {
  try {
    ...
    const result = await nmiAPI.createRecurringSubscription(customerData, subscriptionData);
    res.json(result);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.post('/customers/:id/journey', async (req, res) => {
  try {
    const journey = await xanoAPI.updateLeadJourney(parseInt(req.params.id), req.body);
    res.json(journey);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
