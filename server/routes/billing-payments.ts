...
const NMI_CONFIG = {
  ...
  retryBackoffHours: (process.env.RETRY_BACKOFF_HOURS || "12,36,72")
    .split(",")
    .map((h) => parseInt(h)),
...
router.post("/charge-initial", async (req, res) => {
  try {
    ...
    const transactionData = {
      ...
      descriptor: NMI_CONFIG.descriptorBase,
      retry_attempt: 0,
      decline_reason_category: categorizeDeclineReason(
        responseCode,
        responseTextMsg,
      ),
    ...
    if (isSuccess && customerVaultId) {
      ...
      res.status(400).json({
        ...
        transaction: {
          ...
          response_code: responseCode,
          response_text: responseTextMsg,
          decline_reason: categorizeDeclineReason(
            responseCode,
            responseTextMsg,
          ),
        },
...
router.post("/charge-recurring", async (req, res) => {
  try {
    ...
    if (isSuccess) {
      ...
      console.log(
        "✅ MIT successful - next billing scheduled:",
        nextBillDate.toISOString(),
      );
      ...
      if (newRetryCount >= NMI_CONFIG.maxRetries) {
        ...
        console.log(
          "❌ MIT failed - max retries reached, subscription marked past_due",
        );
      } else {
        // Schedule next retry
        const nextRetryDate = new Date();
        nextRetryDate.setHours(
          nextRetryDate.getHours() +
        ...
        console.log(
          `❌ MIT failed - retry ${newRetryCount} scheduled for:`,
...
router.post("/run-recurring-billing", async (req, res) => {
  try {
    ...
    console.log(
      `✅ Billing run complete: ${successful} successful, ${failed} failed, ${errors} errors`,
    );
...
// Helper functions
function categorizeDeclineReason(
  ...
  switch (responseCode) {
    ...
    case "65":
      return "activity_limit_exceeded";
    case "85":
      return "no_reason_to_decline";
    default:
      if (responseText?.toLowerCase().includes("insufficient"))
        return "insufficient_funds";
