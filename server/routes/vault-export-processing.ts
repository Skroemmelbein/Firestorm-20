...
const VaultExportSchema = z.object({
  export_data: z.object({
    ...
    // Vault records
    vault_records: z.array(z.object({
      ...
      acu_data: z.object({
        ...
        update_source: z.enum(["issuer", "manual", "customer", "acu_service"]).optional(),
      ...
      // Delta tracking
      delta_info: z.object({
        change_type: z.enum(["new", "updated", "deleted", "reactivated"]).optional(),
    ...
    // ACU-specific data
    acu_updates: z.array(z.object({
      customer_vault_id: z.string(),
      update_type: z.enum(["expiry_update", "number_update", "account_closure", "reissue"]),
      old_data: z.object({
    ...
    // Delta changes
    delta_changes: z.array(z.object({
      record_id: z.string(),
...
class VaultExportProcessor implements VaultProcessor {
  ...
  async processVaultExport(exportData: any): Promise<VaultProcessingResult> {
    ...
    for (const vaultRecord of exportData.vault_records) {
      try {
        ...
        failed++;
        validationErrors.push(`Failed to process vault ${vaultRecord.customer_vault_id}: ${error.message}`);
      }
    }
    ...
    let acuResult: ACUProcessingResult = {
      ...
      update_types: { expiry_updates: 0, number_updates: 0, account_closures: 0, reissues: 0 }
    };
  ...
  async processACUUpdates(acuUpdates: any[]): Promise<ACUProcessingResult> {
    ...
    for (const update of acuUpdates) {
      try {
        ...
        result.failed_updates++;
        console.error(`ACU update failed for vault ${update.customer_vault_id}:`, error.message);
      }
    }
  ...
  async processDeltaChanges(deltaChanges: any[]): Promise<DeltaProcessingResult> {
    ...
    for (const change of deltaChanges) {
      try {
        ...
        result.manual_review_required++;
        console.error(`Delta change failed for record ${change.record_id}:`, error.message);
      }
    }
  ...
  validateVaultData(vaultRecord: any): ValidationResult {
    ...
    if (!vaultRecord.customer_vault_id) errors.push("Missing customer_vault_id");
    if (!vaultRecord.customer_id) errors.push("Missing customer_id");
    if (!vaultRecord.customer_info?.email) errors.push("Missing customer email");
    ...
    if (!vaultRecord.payment_method) {
      ...
      if (pm.type === "credit_card") {
        if (!pm.cc_number_masked) errors.push("Missing masked credit card number");
        ...
        if (!pm.account_number_masked) errors.push("Missing masked account number");
    ...
    if (vaultRecord.transaction_summary) {
      ...
      const failureRate = ts.total_transactions > 0 ? 
        ts.failed_transactions / ts.total_transactions : 0;
...
export const validateVaultIntegrity: RequestHandler = async (req, res) => {
  try {
    ...
    const summary = {
      ...
      valid_vaults: validation_results.filter(r => r.status === "VALID").length,
      needs_attention: validation_results.filter(r => r.status === "NEEDS_ATTENTION").length,
      avg_integrity_score: Math.round(
        validation_results.reduce((sum, r) => sum + r.integrity_score, 0) / validation_results.length
      )
    };
