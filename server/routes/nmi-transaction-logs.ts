...
router.post('/get-transaction-logs', async (req, res) => {
  try {
    ...
    if (customer_vault_id) params.append('customer_vault_id', customer_vault_id);
    ...
    console.log('📥 NMI Response received:', responseText.substring(0, 200) + '...');
    ...
    const enhancedTransactions = transactions.map((transaction: any) => ({
      ...
      formatted_date: transaction.time ? new Date(transaction.time).toLocaleString() : 'Unknown',
      
      // Add response categorization
      response_category: categorizeNMIResponse(transaction.response || transaction.response_code),
      is_approved: (transaction.response || transaction.response_code) === '1',
      
      // Add card info if available
      card_type: transaction.cc_type || getCardTypeFromNumber(transaction.cc_number),
      card_last_four: transaction.cc_number ? transaction.cc_number.slice(-4) : null,
...
router.get('/transaction/:transaction_id', async (req, res) => {
  try {
    ...
    if (resultParams.get('response') === '1') {
      ...
      throw new Error(resultParams.get('responsetext') || 'Transaction not found');
    }
...
router.post('/transaction-summary', async (req, res) => {
  try {
    ...
    const transactionResponse = await fetch(`${req.protocol}://${req.get('host')}/api/nmi-logs/get-transaction-logs`, {
      method: 'POST',
    ...
    const summary = {
      ...
      approved_transactions: transactions.filter((t: any) => t.is_approved).length,
      declined_transactions: transactions.filter((t: any) => !t.is_approved).length,
    ...
    if (summary.total_transactions > 0) {
      summary.approval_rate = Math.round((summary.approved_transactions / summary.total_transactions) * 10000) / 100;
    }

    if (summary.approved_transactions > 0) {
      summary.average_transaction_cents = Math.round(summary.total_amount_cents / summary.approved_transactions);
    }
    ...
    transactions.forEach((transaction: any) => {
      // Response codes
      const responseCode = transaction.gateway_response.response_code || 'unknown';
...
function parseNMITransactionResponse(params: URLSearchParams): any[] {
  ...
  while (params.has(`transaction${i}_id`) || params.has(`transactionid${i}`)) {
    const transaction = {
      transactionid: params.get(`transaction${i}_id`) || params.get(`transactionid${i}`),
