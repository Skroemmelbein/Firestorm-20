...
const UpdatedCardsSchema = z.object({
  ...
  card_updates: z.array(z.object({
    ...
    transaction_context: z.object({
      ...
      subscription_status: z.enum(["active", "suspended", "cancelled", "pending"]).optional()
    }).optional(),
...
interface CardUpdateResult {
  ...
  processing_status: "SUCCESS" | "FAILED" | "PENDING_REVIEW" | "REQUIRES_VALIDATION";
...
class UpdatedCardsProcessor implements CardUpdateProcessor {
  ...
  validateCardUpdate(cardUpdate: any): CardUpdateValidation {
    ...
    if (!cardUpdate.updated_card.last_four) errors.push("Missing updated card last four digits");
    if (!cardUpdate.updated_card.exp_month || !cardUpdate.updated_card.exp_year) {
    ...
    const cardNumberChanged = cardUpdate.previous_card.last_four !== cardUpdate.updated_card.last_four;
    const expiryChanged = 
      cardUpdate.previous_card.exp_month !== cardUpdate.updated_card.exp_month ||
      cardUpdate.previous_card.exp_year !== cardUpdate.updated_card.exp_year;
    
    if (cardNumberChanged && cardUpdate.update_details.update_type !== "card_reissue") {
      warnings.push("Card number changed but update type is not 'card_reissue'");
    ...
    const recentFailures = cardUpdate.transaction_context?.recent_failed_attempts || 0;
    if (recentFailures > 3) {
    ...
    if (cardUpdate.update_details.update_type === "account_closure") {
      warnings.push("Account closure notification - billing should be suspended");
    }
    ...
    } else if (cardUpdate.update_details.requires_validation || confidenceAssessment === "low") {
  ...
  async applyCardUpdate(cardUpdate: any): Promise<UpdateApplicationResult> {
    try {
      ...
      console.error(`Failed to apply card update for vault ${cardUpdate.customer_vault_id}:`, error);
      throw error;
    }
  }
  
  async notifyCustomer(cardUpdate: any, result: CardUpdateResult): Promise<void> {
    ...
    console.log(`Sending ${notificationType} notification to ${customer.email} for vault ${cardUpdate.customer_vault_id}`);
  ...
  private async backupPreviousCardData(cardUpdate: any): Promise<boolean> {
    ...
    console.log(`Backing up previous card data for vault ${cardUpdate.customer_vault_id}`);
    return true;
  }
  
  private async updateVaultRecord(cardUpdate: any): Promise<boolean> {
    // Simulate vault update
    console.log(`Updating vault ${cardUpdate.customer_vault_id} with new card data`);
    return true;
  }
  
  private async suspendBilling(vaultId: string): Promise<void> {
    console.log(`Suspending billing for vault ${vaultId} due to account closure`);
  }
  
  private async pauseBillingTemporarily(vaultId: string): Promise<void> {
    console.log(`Temporarily pausing billing for vault ${vaultId} during card update`);
  }
  
  private determineNotificationType(cardUpdate: any, result: CardUpdateResult): string {
...
export const processUpdatedCards: RequestHandler = async (req, res) => {
  try {
    ...
    const summary = {
      ...
      successful_updates: results.filter(r => r.processing_status === "SUCCESS").length,
      failed_updates: results.filter(r => r.processing_status === "FAILED").length,
      pending_review: results.filter(r => r.processing_status === "PENDING_REVIEW").length,
      requires_validation: results.filter(r => r.processing_status === "REQUIRES_VALIDATION").length,
...
export const retryFailedCardUpdates: RequestHandler = async (req, res) => {
  try {
    ...
    const summary = {
      ...
      successful_retries: retryResults.filter(r => r.retry_status === "SUCCESS").length,
      still_failed: retryResults.filter(r => r.retry_status === "FAILED_AGAIN").length
    };
...
export const validateCardUpdateBatch: RequestHandler = async (req, res) => {
  try {
    ...
    const summary = {
      ...
      valid_updates: validationResults.filter(r => r.validation_result.is_valid).length,
      invalid_updates: validationResults.filter(r => !r.validation_result.is_valid).length,
      high_confidence: validationResults.filter(r => r.validation_result.confidence_assessment === "high").length,
      medium_confidence: validationResults.filter(r => r.validation_result.confidence_assessment === "medium").length,
      low_confidence: validationResults.filter(r => r.validation_result.confidence_assessment === "low").length
    };
