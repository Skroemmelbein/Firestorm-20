...
async function processWarChestImport(
  data: z.infer<typeof WarChestImportSchema>,
  ...
  try {
    ...
    console.log(
      `War Chest import completed: ${progress.success_count} success, ${progress.error_count} errors`,
    );
  } catch (error: any) {
...
async function processIndividualClient(client: any, batchId: string) {
  ...
  if (
    !["REWRITE", "FLIP", "BILL", "DORMANT", "DO_NOT_BILL"].includes(
  ...
  console.log(
    `Processed client ${client.client_id} with status ${client.current_status}`,
  );
}
  ...
  try {
    const activeImports = Array.from(importProgress.values())
      ...
      .map((p) => ({
        ...
        completion_percentage: Math.round(
          (p.processed_records / p.total_records) * 100,
        ),
        status: p.status,
