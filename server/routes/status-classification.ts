...
interface ClassificationEngine {
  classifyClient(
    client: z.infer<typeof ClientStatusSchema>,
...
class StatusClassificationEngine implements ClassificationEngine {
  ...
  private calculateRiskScore(
    ...
    // Dispute history
    const activeDisputes = client.dispute_history.filter(
      (d) => d.status === "pending",
    ).length;
  ...
  private shouldNotBill(client: z.infer<typeof ClientStatusSchema>): boolean {
    ...
    if (client.risk_score > 80 && client.compliance_flags.length > 0)
      return true;

    // No TOS acceptance for new billing
    if (!client.tos_acceptance && client.payment_history_months === 0)
      return true;
  ...
  private shouldBill(client: z.infer<typeof ClientStatusSchema>): boolean {
    ...
    if (client.last_payment_date) {
      ...
      if (daysSincePayment <= 45 && client.last_payment_amount! > 0)
        return true;
    }
    ...
    if (
      client.total_lifetime_value > 500 &&
  ...
  private shouldRewrite(client: z.infer<typeof ClientStatusSchema>): boolean {
    ...
    // Plan discontinuation
    const discontinuedPlans = [
      "old_basic",
      "legacy_premium",
      "deprecated_plan",
    ];
    if (client.current_plan && discontinuedPlans.includes(client.current_plan))
      return true;

    // Good payment history but needs plan update
    if (
      client.has_valid_payment_method &&
  ...
  private shouldFlip(client: z.infer<typeof ClientStatusSchema>): boolean {
    ...
    if (
      client.total_lifetime_value > 200 &&
      client.successful_payments > client.failed_payments &&
