...
const DescriptorHistorySchema = z.object({
  ...
  billingPeriod: z
    .enum(["one_time", "monthly", "yearly", "weekly", "daily"])
    .default("monthly"),
  ...
  representmentOutcome: z
    .enum(["won", "lost", "pending", "none"])
    .default("none"),
...
class DescriptorAnalyzer {
  static analyzeDescriptor(descriptor: string): any {
    ...
    const patterns = [
      ...
      {
        name: "subscription",
        regex: /.*(SUB|MEMBERSHIP|MONTHLY|RECURRING).*/i,
      },
    ];
...
class DisputeRiskPredictor {
  static predictDisputeRisk(record: any): any {
    ...
    const descriptorAnalysis = DescriptorAnalyzer.analyzeDescriptor(
    ...
    if (descriptorAnalysis.commonIssues.length > 0) {
      ...
      risk.riskFactors.push(
        `Descriptor issues: ${descriptorAnalysis.commonIssues.join(", ")}`,
      );
    }
    ...
    if (evidenceCount === 0) {
      ...
      risk.protectionFactors.push(
        `Strong evidence portfolio (${evidenceCount} types)`,
      );
    }
    ...
    if (descriptorAnalysis.suggestions.length > 0) {
      risk.recommendations.push(
        ...descriptorAnalysis.suggestions.map((s) => `Descriptor: ${s}`),
      );
    }
...
router.post("/import-record", async (req, res) => {
  try {
    ...
    console.log(
      `📝 Processing descriptor history: ${validatedRecord.transactionId}`,
    );

    // Analyze descriptor and generate insights
    const descriptorAnalysis = DescriptorAnalyzer.analyzeDescriptor(
      validatedRecord.descriptorText,
    );
    const riskAnalysis =
      DisputeRiskPredictor.predictDisputeRisk(validatedRecord);
    const representmentStrength =
...
router.post("/batch-import", async (req, res) => {
  try {
    ...
    console.log(
      `📦 Processing descriptor batch: ${validatedBatch.batchId} with ${validatedBatch.records.length} records`,
    );
    ...
    for (let i = 0; i < validatedBatch.records.length; i++) {
      ...
      try {
        ...
        const representmentStrength =
          DescriptorAnalyzer.generateRepresentmentStrength(record);
        ...
        console.error(
          `❌ Failed to process record ${record.transactionId}:`,
          error.message,
        );
    ...
    results.averageRepresentmentStrength =
      validatedBatch.records.length > 0
    ...
    console.log(
      `✅ Descriptor batch completed: ${results.successful} successful, ${results.failed} failed, ${results.skipped} skipped`,
    );
      ...
      customerDetails:
        customerRecords.length > 0
          ? {
              ...
              billingAddress:
                record.billingAddress || customerRecords[0].billingAddress,
            }
          : null,
      evidenceStrength: {
        overall: record.representmentStrength,
        descriptorQuality:
          record.descriptorAnalysis?.recognizability || "unknown",
        availableEvidence: {
...
router.get("/dispute-readiness/:customerId", async (req, res) => {
  try {
    ...
    const averageRepresentmentStrength = Math.round(
      records.reduce(
        (sum: number, r: any) => sum + (r.representmentStrength || 0),
        0,
      ) / totalTransactions,
    );
    ...
    const strongEvidence = records.filter(
      (r: any) => (r.representmentStrength || 0) >= 70,
    ).length;
    const weakEvidence = records.filter(
      (r: any) => (r.representmentStrength || 0) < 40,
    ).length;
    ...
    const report = {
      ...
      readinessLevel:
        averageRepresentmentStrength >= 70
          ? "high"
          : averageRepresentmentStrength >= 50
            ? "medium"
            : "low",
    ...
    if (weakEvidence > totalTransactions * 0.3) {
      report.vulnerabilities.push(
        "High percentage of weak evidence transactions",
      );
    }

    if (riskDistribution.very_high > 0) {
      report.vulnerabilities.push(
        `${riskDistribution.very_high} very high risk transactions`,
      );
    }
    ...
    const hasOrderConfirmationCount = records.filter(
      (r: any) => r.hasOrderConfirmation,
    ).length;
    ...
    if (weakEvidence > 0) {
      report.recommendations.push(
        "Strengthen evidence collection for weak transactions",
      );
    }

    if (averageRepresentmentStrength < 50) {
      report.recommendations.push(
        "Implement comprehensive dispute prevention strategy",
      );
    }

    const descriptorIssues = records.filter(
      (r: any) =>
    ...
    if (descriptorIssues > totalTransactions * 0.2) {
      report.recommendations.push(
        "Improve descriptor clarity and recognizability",
      );
    }
