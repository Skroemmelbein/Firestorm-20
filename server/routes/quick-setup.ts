...
router.post("/setup-billing-system", async (req, res) => {
  try {
    ...
    const successCount =
      results.tables_created.length + results.plans_seeded.length;
    const errorCount = results.errors.length;

    console.log(
      `✅ Billing system setup complete: ${successCount} items created, ${errorCount} errors`,
    );
...
router.get("/billing-readiness", async (req, res) => {
  try {
    ...
    readiness.ready_for_testing =
      readiness.tables_exist &&
      readiness.plans_exist &&
      readiness.nmi_configured;
