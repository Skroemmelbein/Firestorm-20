  ...
  customers: z.array(
    z.object({
      ...
      legal_identity: z.object({
        ...
        age_verification_status: z
          .enum(["verified", "pending", "failed", "not_required"])
          .optional(),
        identity_verification_level: z
          .enum(["none", "basic", "enhanced", "full_kyc"])
          .default("none"),
      }),
      ...
      contact_info: z.object({
        ...
        email_verification_status: z
          .enum(["verified", "pending", "bounced", "suppressed"])
          .default("pending"),
      ...
      addresses: z
        .array(
          z.object({
            ...
            address_verification_status: z
              .enum(["verified", "invalid", "pending", "partial"])
              .optional(),
      ...
      payment_profile: z
        .object({
          ...
          preferred_payment_method: z
            .enum(["credit_card", "ach", "paypal", "wire"])
            .optional(),
          payment_history_summary: z
            .object({
      ...
      compliance_data: z
        .object({
          ...
          right_to_be_forgotten_requests: z
            .array(
              z.object({
...
class CustomerMasterImportProcessor implements CustomerMasterProcessor {
        ...
        try {
          ...
          if (processing_options.validate_duplicates) {
            ...
            if (duplicateResult.is_duplicate) {
              ...
              if (
                processing_options.merge_duplicates_automatically &&
          ...
          const lifetimeValue =
            customer.relationship_history?.lifetime_value || 0;
          ...
          validationErrors.push(
            `Customer ${customer.customer_id}: ${error.message}`,
          );
        }
      }
  ...
  validateCustomerData(customer: any): CustomerValidationResult {
    ...
    if (!customer.legal_identity?.legal_name?.first_name)
      errors.push("Missing first name");
    if (!customer.legal_identity?.legal_name?.last_name)
      errors.push("Missing last name");
    if (!customer.contact_info?.primary_email)
      errors.push("Missing primary email");
    ...
    completenessScore +=
      (requiredFields.filter(Boolean).length / requiredFields.length) * 60;
    completenessScore +=
      (optionalFields.filter(Boolean).length / optionalFields.length) * 40;
    ...
    let complianceStatus: "compliant" | "needs_review" | "non_compliant" =
      "compliant";
  ...
  detectDuplicates(
    ...
    // Name + phone matching
    const fullName =
      `${customer.legal_identity?.legal_name?.first_name} ${customer.legal_identity?.legal_name?.last_name}`.toLowerCase();
    const customerPhone = customer.contact_info?.primary_phone?.replace(
      /\D/g,
      "",
    );
    ...
    if (customerPhone) {
      const namePhoneMatches = existingCustomers.filter((existing) => {
        const existingName =
          `${existing.legal_identity?.legal_name?.first_name} ${existing.legal_identity?.legal_name?.last_name}`.toLowerCase();
        const existingPhone = existing.contact_info?.primary_phone?.replace(
          /\D/g,
          "",
        );
      ...
      namePhoneMatches.forEach((match) => {
        if (
          !matchingCustomers.find((m) => m.customer_id === match.customer_id)
        ) {
          matchingCustomers.push({
            customer_id: match.customer_id,
            match_type: "exact",
    ...
    fuzzyMatches.forEach((match) => {
      if (!matchingCustomers.find((m) => m.customer_id === match.customer_id)) {
        const existingName =
          `${match.legal_identity?.legal_name?.first_name} ${match.legal_identity?.legal_name?.last_name}`.toLowerCase();
    ...
    const highestConfidence = Math.max(
      ...matchingCustomers.map((m) => m.similarity_score),
    );

    let recommendedAction: DuplicateDetectionResult["recommended_action"] =
      "create_new";
    if (isDuplicate) {
  ...
  async enrichCustomerData(customer: any): Promise<CustomerEnrichmentResult> {
    ...
    if (customer.addresses?.length > 0) {
      ...
      if (primaryAddress?.postal_code) {
        enrichedFields.geographic_data = await this.enrichGeographicData(
    ...
    if (customer.legal_identity?.date_of_birth) {
      enrichedFields.demographic_data = this.enrichDemographicData(
        customer.legal_identity.date_of_birth,
      );
...
export const startCustomerMasterImport: RequestHandler = async (req, res) => {
  try {
    ...
    const result = await customerProcessor.processCustomerImport(
      validation.data,
    );
...
export const validateCustomerData: RequestHandler = async (req, res) => {
  try {
    ...
    const summary = {
      ...
      valid_customers: validationResults.filter(
        (r) => r.validation_result.is_valid,
      ).length,
      invalid_customers: validationResults.filter(
        (r) => !r.validation_result.is_valid,
      ).length,
      avg_completeness_score: Math.round(
        validationResults.reduce(
          (sum, r) => sum + r.validation_result.completeness_score,
          0,
        ) / validationResults.length,
      ),
      avg_accuracy_score: Math.round(
        validationResults.reduce(
          (sum, r) => sum + r.validation_result.accuracy_score,
          0,
        ) / validationResults.length,
      ),
      compliance_summary: {
        compliant: validationResults.filter(
          (r) => r.validation_result.compliance_status === "compliant",
        ).length,
        needs_review: validationResults.filter(
          (r) => r.validation_result.compliance_status === "needs_review",
        ).length,
        non_compliant: validationResults.filter(
          (r) => r.validation_result.compliance_status === "non_compliant",
        ).length,
      },
    };
