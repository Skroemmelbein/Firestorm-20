...
const CustomerMasterSchema = z.object({
  ...
  // Legal & Compliance
  kycStatus: z.enum(["verified", "pending", "failed", "not_required"]).default("pending"),
  amlStatus: z.enum(["clear", "flagged", "under_review", "rejected"]).default("clear"),
  ...
  bankAccountType: z.enum(["checking", "savings", "business", "unknown"]).optional(),
  ...
  // Account Status
  accountStatus: z.enum(["active", "suspended", "closed", "pending_verification"]).default("active"),
  ...
  communicationFrequency: z.enum(["daily", "weekly", "monthly", "never"]).default("weekly"),
  ...
  verificationMethod: z.enum(["manual", "automated", "third_party", "none"]).default("none"),
...
class DuplicateDetector {
  static async findPotentialDuplicates(customer: any): Promise<any[]> {
    ...
    try {
      ...
      potentialDupes.push(...emailMatches.map((m: any) => ({ ...m, matchType: "email_exact", confidence: 0.95 })));
    } catch (e) {}
    ...
    try {
      ...
      potentialDupes.push(...phoneMatches.map((m: any) => ({ ...m, matchType: "phone_exact", confidence: 0.90 })));
    } catch (e) {}
    
    // Name + Address fuzzy match
    try {
      const nameAddressMatches = await xanoAPI.queryRecords("customer_master", {});
      const fuzzyMatches = nameAddressMatches.filter((existing: any) => {
        ...
        const addressScore = this.calculateAddressSimilarity(customer.billingAddress, existing.billingAddress);
        return nameScore > 0.8 && addressScore > 0.7;
      });
      potentialDupes.push(...fuzzyMatches.map((m: any) => ({ ...m, matchType: "name_address_fuzzy", confidence: 0.75 })));
    } catch (e) {}
    ...
    if (customer.socialSecurityLastFour) {
      try {
        ...
        potentialDupes.push(...ssnMatches.map((m: any) => ({ ...m, matchType: "ssn_last4", confidence: 0.85 })));
      } catch (e) {}
    }
    
    // Remove exact duplicates and sort by confidence
    const uniqueDupes = potentialDupes.filter((dupe, index, self) => 
  ...
  private static calculateAddressSimilarity(addr1: any, addr2: any): number {
    ...
    const cityScore = normalize(addr1.city) === normalize(addr2.city) ? 1.0 : 0.0;
    const stateScore = addr1.state === addr2.state ? 1.0 : 0.0;
    const zipScore = addr1.zipCode === addr2.zipCode ? 1.0 : 0.0;
    
    return (street1Score * 0.4 + cityScore * 0.2 + stateScore * 0.2 + zipScore * 0.2);
  }
}
...
class DataQualityScorer {
  static calculateScore(customer: any): number {
    ...
    if (customer.emailAddress?.includes('@') && customer.emailAddress?.includes('.')) score += 10;
...
class DataEnrichmentEngine {
  static async enrichCustomerData(customer: any): Promise<any> {
    ...
    if (!enriched.timezone && enriched.billingAddress?.state) {
      enriched.timezone = this.inferTimezoneFromState(enriched.billingAddress.state);
    }
  ...
  private static inferTimezoneFromState(state: string): string {
    const timezoneMap: Record<string, string> = {
      ...
      'AZ': 'America/Phoenix', 'NV': 'America/Los_Angeles', 'UT': 'America/Denver',
      ...
      'OH': 'America/New_York', 'MI': 'America/New_York', 'GA': 'America/New_York',
      'FL': 'America/New_York', 'SC': 'America/New_York', 'NC': 'America/New_York',
      'VA': 'America/New_York', 'WV': 'America/New_York', 'MD': 'America/New_York',
      'DE': 'America/New_York', 'PA': 'America/New_York', 'NJ': 'America/New_York',
      'NY': 'America/New_York', 'CT': 'America/New_York', 'RI': 'America/New_York',
      'MA': 'America/New_York', 'VT': 'America/New_York', 'NH': 'America/New_York',
      'ME': 'America/New_York', 'AK': 'America/Anchorage', 'HI': 'Pacific/Honolulu'
    };
...
router.post("/import-customer", async (req, res) => {
  try {
    ...
    const duplicates = await DuplicateDetector.findPotentialDuplicates(validatedCustomer);
    ...
    const enrichedCustomer = await DataEnrichmentEngine.enrichCustomerData(validatedCustomer);
    ...
    // Save to Xano
    const savedCustomer = await xanoAPI.createRecord("customer_master", enrichedCustomer);
...
router.post("/batch-import", async (req, res) => {
  try {
    ...
    console.log(`📦 Processing batch: ${validatedBatch.batchId} with ${validatedBatch.customers.length} customers`);
    ...
    for (let i = 0; i < validatedBatch.customers.length; i++) {
      ...
      try {
        if (validatedBatch.processingMode === "validate_only") {
          ...
          const enriched = await DataEnrichmentEngine.enrichCustomerData(customer);
          const duplicates = await DuplicateDetector.findPotentialDuplicates(customer);
          ...
          if (highConfidenceDupe && validatedBatch.duplicateHandling === "skip") {
            results.results.push({
              customerId: customer.customerId,
              status: "skipped",
          ...
          if (highConfidenceDupe && validatedBatch.duplicateHandling === "error") {
            results.results.push({
              customerId: customer.customerId,
              status: "failed",
              reason: "duplicate_detected",
          ...
          if (highConfidenceDupe && validatedBatch.duplicateHandling === "merge") {
            ...
            savedCustomer = await xanoAPI.updateRecord("customer_master", highConfidenceDupe.id, mergedData);
            ...
            // Create new record
            savedCustomer = await xanoAPI.createRecord("customer_master", enriched);
        ...
        if ((i + 1) % 100 === 0) {
          console.log(`📊 Progress: ${i + 1}/${validatedBatch.customers.length} customers processed`);
        ...
        console.error(`❌ Failed to process customer ${customer.customerId}:`, error.message);
    ...
    console.log(`✅ Batch completed: ${results.successful} successful, ${results.failed} failed, ${results.skipped} skipped`);
...
router.get("/batch/:batchId", async (req, res) => {
  try {
    ...
    const qualityMetrics = {
      averageDataQuality: customers.length > 0 
        ? customers.reduce((sum: number, c: any) => sum + (c.dataQualityScore || 0), 0) / customers.length 
        : 0,
      highQualityCount: customers.filter((c: any) => (c.dataQualityScore || 0) >= 80).length,
      verifiedCount: customers.filter((c: any) => c.verificationMethod !== "none").length,
