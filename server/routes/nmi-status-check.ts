    ...
    if (resultParams.get("response") === "1") {
      const accountInfo = {
        ...
        last_transaction_time:
          resultParams.get("last_transaction_time") || "Unknown",
      ...
      throw new Error(
        resultParams.get("responsetext") || "Account status check failed",
      );
    }
  } catch (error: any) {
...
router.post("/validate-connection", async (req, res) => {
  try {
    ...
    const isValidCredentials =
      response.ok && responseText.includes("response=");
...
function generateRecommendations(accountInfo: any): string[] {
  ...
  if (
    parseInt(accountInfo.current_daily_volume) >
    parseInt(accountInfo.daily_limit) * 0.8
  ) {
    recommendations.push(
      "Daily volume is approaching limit. Consider upgrading account or waiting until tomorrow.",
    );
  }

  if (accountInfo.account_type === "Test") {
    recommendations.push(
      "This appears to be a test account. Contact NMI to upgrade to production for higher limits.",
    );
  }
...
function generateActivitySuggestions(activity: any): string[] {
  ...
  if (transactionCount > 50) {
    suggestions.push(
      "High transaction volume detected. If testing, consider spacing out transactions.",
    );
  }

  if (activity.last_decline_reason.includes("limit")) {
    suggestions.push(
      "Recent declines due to limits. Wait before attempting more transactions.",
    );
  }

  if (activity.status === "limited") {
    suggestions.push(
      "Account currently limited. Contact NMI support for assistance.",
    );
  }
