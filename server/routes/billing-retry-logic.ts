...
const RETRY_CONFIG = {
  ...
  backoffHours: (process.env.RETRY_BACKOFF_HOURS || "12,36,72")
    .split(",")
    .map((h) => parseInt(h)),
  ...
  categorizeDecline(
    ...
    const declineMap: Record<string, any> = {
      ...
      "05": {
        ...
        actionRequired: "retry_with_variation",
      },
      "14": {
        category: "invalid_card",
        severity: "high",
        retryRecommended: false,
        actionRequired: "update_card",
      },
      "51": {
      ...
      "61": {
        ...
        severity: "medium",
        retryRecommended: true,
        actionRequired: "retry_smaller_amount",
      },
      "65": {
      ...
      "78": {
        category: "card_not_activated",
        severity: "high",
        retryRecommended: false,
        actionRequired: "customer_contact",
      },
      "85": {
...
router.post("/schedule-retry", async (req, res) => {
  try {
    const { subscription_id, decline_reason, response_code, response_text } =
      req.body;
    ...
    const declineAnalysis = retryEngine.categorizeDecline(
      response_code,
      response_text,
    );
    ...
    if (!retryEngine.shouldRetry(subscription, response_code)) {
      console.log(
        `🚫 Retry not recommended for subscription ${subscription_id}, reason: ${response_code}`,
      );
      ...
      await getConvexClient().updateRecord("subscriptions", subscription_id, {
        status: newStatus,
    ...
    const descriptor = retryEngine.generateDescriptor(
      newRetryCount,
      response_code,
    ...
    const retrySchedule = await getConvexClient().createRecord(
      "retry_schedule",
      retryScheduleData,
    );

    // Log decline insight for analytics
    await logDeclineInsight(
      response_code,
      response_text,
      subscription.card_brand,
      `retry_${newRetryCount}`,
    );

    console.log(
      `⏰ Retry ${newRetryCount} scheduled for subscription ${subscription_id} at ${retryTime.toISOString()}`,
    );
...
router.post("/process-retries", async (req, res) => {
  try {
    ...
    const dueRetries = await getConvexClient().queryRecords("retry_schedule", {
      status: "pending",
      scheduled_at: { "<=": now },
    });
    ...
    for (const retry of dueRetries) {
      try {
        ...
        await getConvexClient().updateRecord("retry_schedule", retry.id, {
          status: "executed",
        ...
        const response = await fetch(
          `${req.protocol}://${req.get("host")}/api/billing/charge-recurring`,
          {
            method: "POST",
        ...
        console.log(
          `${result.success ? "✅" : "❌"} Retry ${retry.retry_attempt} for subscription ${retry.subscription_id}: ${result.message}`,
        );
    ...
    console.log(
      `✅ Retry processing complete: ${successful} successful, ${failed} failed, ${errors} errors`,
    );
...
router.get("/retry-analytics", async (req, res) => {
  try {
    ...
    const retries = await getConvexClient().queryRecords("retry_schedule", {
...
async function logDeclineInsight(
  responseCode: string,
  responseText: string,
  cardBrand: string,
  retryStage: string,
  ...
  try {
    ...
    const existing = await getConvexClient().queryRecords("decline_insights", {
      date: today,
    ...
    if (existing.length > 0) {
      ...
      await getConvexClient().updateRecord("decline_insights", existing[0].id, {
        decline_count: existing[0].decline_count + 1,
      });
    } else {
      // Create new entry
      await getConvexClient().createRecord("decline_insights", {
        date: today,
        response_code: responseCode,
