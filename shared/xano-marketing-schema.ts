...
export interface XanoMarketingSchema {
  ...
  message_templates: {
    ...
    category: 'promotional' | 'transactional' | 'abandoned_cart' | 'welcome' | 'retention';
    ...
    ai_goal: 'awareness' | 'consideration' | 'conversion' | 'retention' | 'support';
  ...
  ai_responses: {
    ...
    uuid: string;
    name: string;
    trigger_type: 'keyword' | 'intent' | 'sentiment' | 'time_based' | 'behavioral';
  ...
  automation_flows: {
    ...
    name: string;
    description: string;
    trigger_type: 'signup' | 'purchase' | 'abandoned_cart' | 'birthday' | 'anniversary' | 'custom';
  ...
  compliance_logs: {
    id: number;
    customer_id: number; // FK to customers
    action_type: 'opt_in' | 'opt_out' | 'data_request' | 'data_deletion' | 'consent_update';
